...
class TestSessionOwnershipValidation:
    ...
    def setup_method(self):
        ...
        self.bitcoin_service_patcher = patch(
            "src.web.bitcoin_assistant_api.bitcoin_service", self.mock_bitcoin_service
        )
    ...
    def test_delete_session_with_wrong_session_id(self):
        ...
        assert (
            "Forbidden: You can only delete your own session"
    ...
    def test_delete_session_with_valid_ownership(self):
        ...
        response = self.client.delete(
            f"/session/{session_id}", cookies={"btc_assistant_session": session_id}
        )
    ...
    def test_get_session_info_with_wrong_session_id(self):
        ...
        assert response.status_code == 403
        assert (
            "Forbidden: You can only access your own session information"
            in response.json()["detail"]
        )
    ...
    def test_session_ownership_prevents_cross_user_access(self):
        ...
        response = self.client.get(
            f"/session/{bob_session}", cookies={"btc_assistant_session": alice_session}
        )
