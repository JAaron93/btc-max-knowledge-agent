...
class TestHighConcurrencySimulation:
    ...
    async def test_concurrent_synthesis_requests(self, tts_service):
        ...
        with patch("aiohttp.ClientSession", return_value=mock_session):
            ...
            assert (
                len(successful_results) + len(failed_results) + len(exceptions)
                == num_requests
            )
    ...
    async def test_concurrent_cache_access(self, tts_service):
        ...
        num_tasks = 30
        cache_keys = [
            f"cache_test_{i}" for i in range(10)
        ]  # Shared keys for contention
...
class TestMemoryLeakDetection:
    ...
    async def test_continuous_synthesis_memory_usage(self, tts_service_with_cache):
        ...
        if baseline_memory > 0:
            ...
            assert (
                max_memory < baseline_memory + 150
...
class TestCacheContentionTesting:
    ...
    def test_concurrent_read_write_operations(self, multi_cache):
        ...
        with ThreadPoolExecutor(max_workers=num_threads) as executor:
            # Submit all tasks
            futures = [
                executor.submit(cache_worker, thread_id, operation_type)
        ...
        success_rate = (
            len(successful_operations) / total_operations if total_operations > 0 else 0
        )
        ...
        if successful_operations:
            avg_operation_time = statistics.mean(
                [r["time"] for r in successful_operations]
            )
            assert (
                avg_operation_time < 0.1
            ), f"Average operation time: {avg_operation_time:.3f}s"
    ...
    async def test_response_times_under_load(self, benchmark_service):
        ...
        for load_level, results in benchmark_results.items():
            if results["successful_requests"] > 0:
                ...
                ), f"Load {load_level}: Success rate {results['success_rate']:.2%}"

                # Response times should be reasonable
                assert (
                    results["avg_response_time"] < 2.0
                ), f"Load {load_level}: Avg response time {results['avg_response_time']:.3f}s"
...
class TestThreadSafetyValidation:
        ...
        def volume_worker(thread_id):
            ...
            for i in range(operations_per_thread):
                try:
                    ...
                    assert (
                        0.1 <= volume <= 0.9
                    ), f"Volume {volume} should be between 0.1 and 0.9"
                    ...
                    assert (
                        0.0 <= current_volume <= 1.0
                    ), f"Retrieved volume {current_volume} should be between 0.0 and 1.0"
    ...
    def test_volume_validation_and_error_handling(self, thread_safe_service):
        ...
        for volume in valid_volumes:
            ...
            assert (
                retrieved == volume
            ), f"Volume {volume} was not set correctly, got {retrieved}"
        ...
        final_volume = thread_safe_service.get_volume()
        assert (
            0.0 <= final_volume <= 1.0
        ), f"Service volume corrupted after invalid attempts: {final_volume}"

        # Test that valid volume can still be set after invalid attempts
        thread_safe_service.set_volume(0.7)
        assert (
            thread_safe_service.get_volume() == 0.7
        ), "Valid volume setting failed after invalid attempts"
    ...
    def test_concurrent_error_state_access(self, thread_safe_service):
        ...
        assert (
            success_rate > 0.98
        ), f"Error state access success rate: {success_rate:.2%}"
