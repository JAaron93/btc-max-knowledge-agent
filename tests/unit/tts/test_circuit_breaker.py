...
async def test_circuit_breaker_basic():
    ...
    logger.info("Circuit state after failures: %s", circuit_state)

    if circuit_state["state"] == CircuitState.OPEN.value:
        logger.info("✓ Circuit opened after failure threshold reached")
    else:
        logger.warning("⚠ Circuit state is %s, expected 'open'", circuit_state["state"])
    ...
    try:
        _ = await error_handler.execute_with_retry(
            simulate_successful_operation
        )
        ...
        if circuit_state["state"] == CircuitState.CLOSED.value:
            ...
            logger.warning(
                "⚠ Circuit state is %s, expected 'closed'", circuit_state["state"]
            )
...
async def test_circuit_breaker_failure_during_recovery():
    ...
    logger.info("=== Testing Failure During Recovery ===")
    ...
    try:
        ...
        logger.info("Recovery attempt failed as expected: %s", type(e).__name__)
...
async def test_timeout_handling():
    ...
    try:
        ...
        # depending on policy.
        logger.info("✓ Timeout handled correctly: %s: %s", type(e).__name__, e)
...
async def main():
    ...
    try:
        ...
        logger.error("Test failed with error: %s: %s", type(e).__name__, e)
        raise
