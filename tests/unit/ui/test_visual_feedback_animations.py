...
def match_attribute_flexible(
    html: str, tag: str, attribute: str, expected_value: str, allow_quotes: bool = True
) -> bool:
...
def get_tts_status_display(
    ...
    if has_error:
        ...
        if error_info:
            ...
            display_message = error_type_messages.get(
                error_type, "[ERROR] TTS Error - Text continues normally"
            )
        else:
...
def test_waveform_animation():
    ...
    assert "svg" in animation_html.lower(), (
        f"Animation should contain SVG element, but got: {animation_html[:100]}..."
    )
...
def test_smooth_transitions():
    ...
    states = [
        ...
        (
            ...
            get_tts_status_display(
                False, has_error=True, error_info={"error_type": "TEST"}
            ),
        ),
    ]
    ...
    for state_name, state_html in states:
        ...
        if state_name in [
            "Ready",
            "Error",
        ]:  # Ready and Error states should have fade-in
...
def test_visual_requirements_compliance():
    ...
    cached_status = get_tts_status_display(
...
def test_html_structure_validation():
    ...
    for test_name, html in test_cases:
        ...
        if div_count > 0:
            print(
                f"  ✓ {test_name}: Valid HTML with {div_count} div(s), {span_count} span(s)"
            )
        else:
