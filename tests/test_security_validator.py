...
class TestSecurityValidator:
    ...
    def test_initialization(self, validator):
        ...
        assert len(validator._compiled_patterns) == len(
    ...
    async def test_input_length_boundary_conditions(self, validator):
        ...
        result_over_limit = await validator.validate_input(
        ...
        length_violations_emoji = [
            v
            for v in result_emoji_at_limit.violations
            if v.violation_type == "input_length_exceeded"
        ]
        ...
        length_violations_emoji_over = [
            v
            for v in result_emoji_over.violations
            if v.violation_type == "input_length_exceeded"
        ]
    ...
    async def test_utf8_validation_valid_input(self, validator):
        ...
        for test_input in test_inputs:
            ...
            utf8_violations = [
                ...
                if v.violation_type == "invalid_utf8_encoding"
            ]
            assert (
                len(utf8_violations) == 0
            ), f"Valid UTF-8 input '{test_input}' should not have encoding violations"
    ...
    def test_utf8_validation_edge_cases(self, validator):
        ...
        for test_input, description in test_cases:
            ...
            assert (
                violation is None
            ), f"Valid UTF-8 input should not produce violations: {description}"
    ...
    def test_utf8_validation_error_handling(self, validator):
        ...
        with patch("codecs.decode", side_effect=mock_error):
            ...
            ), "Mocked UTF-8 error should produce a violation"
        ...
        with patch("codecs.decode", side_effect=Exception("General encoding error")):
            ...
            assert (
                violation is not None
            ), "General encoding error should produce a violation"
    ...
    async def test_libinjection_integration_sql_injection(
        self, mock_libinjection, validator
    ):
        ...
        sql_violations = [
            v for v in result.violations if v.violation_type == "sql_injection"
        ]
    ...
    async def test_libinjection_integration_xss(self, mock_libinjection, validator):
        ...
        xss_violations = [
            v for v in result.violations if v.violation_type == "xss_injection"
        ]
    ...
    @patch("src.security.validator.LIBINJECTION_AVAILABLE", True)
    @patch("src.security.validator.libinjection")
    async def test_libinjection_graceful_degradation(
        self, mock_libinjection, validator
    ):
        ...
        degradation_violations = [
            v for v in result.violations if v.violation_type == "library_degradation"
        ]
        assert len(degradation_violations) == 1
        assert degradation_violations[0].severity == SecuritySeverity.WARNING
        assert (
            "libinjection detection unavailable"
            in degradation_violations[0].description
        )
    ...
    async def test_sanitization_fallback_without_libraries(self, security_config):
        ...
        with patch("src.security.validator.BLEACH_AVAILABLE", False), patch(
            "src.security.validator.MARKUPSAFE_AVAILABLE", False
        ):
            ...
            assert (
                "DROP TABLE" not in result or "\\'" in result
    ...
    async def test_query_parameter_validation(self, validator):
        ...
        assert not result.is_valid
        top_k_violations = [
            v for v in result.violations if v.violation_type == "invalid_top_k"
        ]
        ...
        threshold_violations = [
            v
            for v in result.violations
            if v.violation_type == "invalid_similarity_threshold"
        ]
        ...
        vector_violations = [
            v
            for v in result.violations
            if v.violation_type == "vector_dimensions_exceeded"
        ]
    ...
    def test_recommended_action_determination(self, validator):
        ...
        assert (
            validator._determine_recommended_action([low_severity])
    ...
    async def test_error_handling_during_validation(self, validator):
        ...
        with patch.object(
            validator, "_validate_input_length", side_effect=Exception("Test error")
        ):
            test_input = "test input"
            context = {"source_ip": "127.0.0.1"}
