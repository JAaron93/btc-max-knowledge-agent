...
class TestLoggingPerformance:
    ...
    def test_debug_logging_performance_old_vs_new(self):
        ...
        start_time = time.time()
        for i in range(iterations):
            new_logger.debug_lazy(
                lambda i=i: f"Processing item {i}: {self.expensive_operation(i)}"
            )
        ...
        print(
            f"Performance improvement: {((old_time - new_time) / old_time * 100):.1f}%"
        )

        # New logging should be significantly faster when debug is disabled
        assert (
            new_time < old_time * 0.5
        ), f"Expected at least 50% improvement, got {new_time:.3f}s vs {old_time:.3f}s"
    ...
    def test_lazy_logging_performance(self):
        ...
        print(
            f"Lazy improvement: {((regular_time - lazy_time) / regular_time * 100):.1f}%"
        )
    ...
    def test_conditional_logging_performance(self):
        ...
        print(
            f"Conditional improvement: {((without_condition_time - with_condition_time) / without_condition_time * 100):.1f}%"
        )

        # Conditional logging should be much faster
        assert (
            with_condition_time < without_condition_time * 0.2
        ), "Expected conditional logging to be 80% faster"
    ...
    def test_url_metadata_logging_performance(self):
        ...
        for i in range(iterations):
            log_validation_optimized(
                test_url, True, "url_format", details={"test": f"data_{i}"}
            )
        new_time = time.time() - start_time
        ...
        print(
            f"URL logging improvement: {((old_time - new_time) / old_time * 100):.1f}%"
        )

        # New logging should be faster
        assert (
            new_time < old_time * 0.8
        ), "Expected at least 20% improvement in URL logging"
    ...
    def test_memory_usage_optimization(self):
        ...
        assert (
            objects_created < iterations * 0.1
        ), f"Too many objects created: {objects_created}"
    ...
    def test_production_environment_optimization(self):
        ...
        print(
            f"Average time per log call: {(production_time / iterations * 1000):.3f}ms"
        )

        # Should be very fast (less than 1ms per call on average)
        assert (
            production_time < iterations * 0.001
        ), f"Production logging too slow: {production_time:.3f}s"
...
class TestLoggingMemoryLeaks:
    ...
    def test_no_memory_leaks_in_disabled_logging(self):
        ...
        logger = PerformanceOptimizedLogger(
            "test_leaks", "ERROR"
        )  # Most logging disabled
        ...
        assert (
            alive_objects == 0
        ), f"Memory leak detected: {alive_objects} objects still referenced"
