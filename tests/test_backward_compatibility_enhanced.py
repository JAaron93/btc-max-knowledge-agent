...
class TestBackwardCompatibilityEnhanced:
    ...
    def test_query_legacy_vectors_only(self, pinecone_client, mock_pinecone_index):
        ...
        # Perform query
        results = pinecone_client.query(vector=np.random.rand(1536).tolist(), top_k=2)
    ...
    def test_query_mixed_vectors(self, pinecone_client, mock_pinecone_index):
        ...
        # Perform query
        results = pinecone_client.query(vector=np.random.rand(1536).tolist(), top_k=4)
    ...
    def test_retroactive_url_enrichment(self, pinecone_client, mock_pinecone_index):
        ...
        # Simulate update operation (PineconeClient doesn't have index attribute exposed)
        # We would normally call update through the index
        index = pinecone_client.get_index()
        index.update(id=vector_id, metadata=enriched_metadata)
    ...
    def test_assistant_agent_backward_compatibility(self, mock_pinecone_index):
        ...
        with patch(
            "src.agents.pinecone_assistant_agent.PineconeClient"
            ...
            agent = PineconeAssistantAgent(
                assistant_id="test-assistant", pinecone_index_name="test-index"
            )
                ...
                with patch.object(agent.client.beta.threads.messages, "create"):
                    with patch.object(
                        ...
                        mock_run.return_value = Mock(
                            status="completed", thread_id="thread-123"
                        )
                        ...
                        with patch.object(
                            agent.client.beta.threads.messages, "list"
                        ) as mock_list:
    ...
    def test_concurrent_access_mixed_vectors(
        self, pinecone_client, mock_pinecone_index
    ):
        ...
        def query_worker(client, query_vector, worker_id):
            ...
            try:
                result = client.query(vector=query_vector, top_k=5)
        ...
        for i in range(num_workers):
            ...
            thread = threading.Thread(
                target=query_worker, args=(pinecone_client, query_vector, i)
            )
    ...
    def test_performance_impact_measurement(
        self, pinecone_client, mock_pinecone_index, capsys
    ):
        ...
        pinecone_client.query(vector=np.random.rand(1536).tolist(), top_k=100)
        ...
        ), "Modern queries should not be significantly slower"
...
class TestMigrationStrategies:
    ...
    def test_rollback_capability(self):
        ...
        backup = copy.deepcopy(original_vector["metadata"])

        # Add URL metadata (use deep copy to avoid modifying original)
        modified_vector = copy.deepcopy(original_vector)
        ...
        assert "source_url" in modified_vector["metadata"]
        assert modified_vector["metadata"] != original_vector["metadata"]

        # Simulate rollback by resetting metadata to backup
        modified_vector["metadata"] = backup.copy()

        # Verify rollback
        assert modified_vector["metadata"] == original_vector["metadata"]
        assert "source_url" not in modified_vector["metadata"]
