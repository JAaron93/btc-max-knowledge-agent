...
class TestSecurityConfiguration:
    ...
    def test_default_configuration_is_valid(self):
        ...
        assert (
            len(errors) == 0
        ), f"Default configuration should be valid, but got errors: {errors}"
    ...
    def test_invalid_configuration_detection(self):
        ...
        assert (
            "injection_detection_threshold must be between 0.0 and 1.0"
            in error_messages
        )
    ...
    def test_api_key_length_validation(self):
        ...
        config = SecurityConfiguration(
            api_key_min_length=64, api_key_max_length=32  # Invalid: min > max
        )

        errors = config.validate()
        assert any(
            "api_key_min_length must be less than api_key_max_length" in error
            for error in errors
        )
    ...
    def test_environment_validation(self):
        ...
        errors = config.validate()
        assert any(
            "environment must be one of: development, staging, production" in error
            for error in errors
        )
...
class TestSecurityConfigurationManager:
    ...
    def test_validate_environment_variables(self, config_manager):
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            assert any(
                v.violation_type == "missing_environment_variable"
...
class TestSecurityModels:
    ...
    def test_auth_result_risk_score_validation(self):
        ...
        auth_result = AuthResult(
            status=AuthenticationStatus.SUCCESS, client_id="test_client", risk_score=0.3
        )
