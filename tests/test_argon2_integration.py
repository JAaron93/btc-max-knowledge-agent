...
class TestArgon2Integration:
    ...
    def setup_method(self):
        ...
        # Generate a real Argon2id hash for "admin123" using the same library
        from argon2 import PasswordHasher

        hasher = PasswordHasher()
        test_password_hash = hasher.hash("admin123")

        # Set up environment variables for AdminAuthenticator
    ...
    def test_password_verification_security(self):
        ...
        token3 = self.authenticator.authenticate_admin("test_admin", "", "127.0.0.1")
    ...
    def test_hash_uniqueness(self):
        ...
        hasher = PasswordHasher()
        test_hash = hasher.hash("admin123")

        # Set up environment variables for both authenticators
        with patch.dict(
            "os.environ",
            {
                "ADMIN_USERNAME": "test_admin",
                "ADMIN_PASSWORD_HASH": test_hash,
                "ADMIN_SECRET_KEY": "test_secret_key_32_chars_long_12345",
            },
            clear=False,
        ):
            # Create two authenticators with the same configuration
            auth1 = AdminAuthenticator()
            auth2 = AdminAuthenticator()

            # Both should work with the same password
            token1 = auth1.authenticate_admin("test_admin", "admin123", "127.0.0.1")
            token2 = auth2.authenticate_admin("test_admin", "admin123", "127.0.0.2")

            assert token1 is not None
            assert token2 is not None

            # Both authenticators use the same configured hash
            assert auth1.admin_password_hash == auth2.admin_password_hash
            assert auth1.admin_password_hash.startswith("$argon2id$")
