...
class TestTTSServiceUnit:
    ...
    async def test_synthesize_text_with_cache(self, tts_service):
        ...
        with patch.object(
            tts_service, "get_cached_audio", return_value=None
        ), patch.object(
            tts_service.error_handler, "execute_with_retry", return_value=test_audio
        ) as mock_retry, patch.object(
            tts_service, "cache_audio", return_value="cache_key"
        ):
...
class TestTextCleaningAlgorithm:
    ...
    def test_special_character_handling(self):
        ...
        for input_text, expected_content in test_cases:
            ...
            assert (
                expected_content in result
            ), f"Expected '{expected_content}' in result '{result}'"
    ...
    def test_edge_cases(self):
        ...
        *Source: [Bitcoin.org](https://bitcoin.org)*
        ...
        )  # Should clean up broken link syntax
