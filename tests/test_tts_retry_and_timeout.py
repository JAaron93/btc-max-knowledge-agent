...
from src.utils.tts_error_handler import (
    TTSErrorHandler, TTSError, TTSAPIKeyError, TTSRateLimitError, 
    TTSServerError, TTSNetworkError, TTSRetryExhaustedError,
...
class TestTimeoutHandling:
    ...
    async def test_timeout_retry_behavior(self, tts_service):
        ...
        with patch.object(tts_service.error_handler, 'execute_with_retry') as mock_retry:
    ...
    async def test_timeout_with_circuit_breaker(self, tts_service):
        ...
        assert circuit_state['failure_count'] > 0, "Circuit breaker should have recorded failures"
        assert circuit_state['failure_rate'] > 0, "Circuit breaker should show failure rate > 0"
        
        # Circuit may be open or closed depending on exact timing and configuration,
        # but it should have recorded the failure pattern
        assert circuit_state['state'] in ['closed', 'open', 'half_open'], f"Unexpected circuit state: {circuit_state['state']}"
        
        # If enough failures occurred within the window, circuit should be open
        if circuit_state['failure_rate'] >= 0.5:  # Default threshold
            assert circuit_state['state'] == 'open', "Circuit should be open with high failure rate"
...
class TestNetworkErrorScenarios:
        ...
        async def network_recovery_response(*args, **kwargs):
            ...
            if call_count == 1:
                ...
                raise aiohttp.ClientConnectorError(
                    connection_key=None, os_error=ConnectionError("Network temporarily unavailable")
                )
            else:
                # Second call: success
                mock_response = AsyncMock()
    ...
    async def test_network_error_retry_limits(self, tts_service):
        ...
        expected_attempts = tts_service.error_handler.retry_config['max_retries_5xx'] + 1
        assert call_count == expected_attempts
