...
from src.utils.audio_utils import (
    AudioFormatConverter, AudioStreamingManager, get_audio_streaming_manager,
    AudioStreamProcessor
)
from src.utils.tts_error_handler import TTSErrorHandler, TTSNetworkError, TTSServerError
...
class TestAudioStreamingPerformance:
    ...
    def test_streaming_chunk_sizes(self, streaming_manager):
        ...
        for chunk_size in chunk_sizes:
            ...
            performance_results[chunk_size] = {
                ...
                'chunk_efficiency': len(test_audio) / (num_chunks * chunk_size) if num_chunks > 0 else 0
            }
        
        # Verify performance characteristics
        for chunk_size, results in performance_results.items():
            # Processing should be fast
            assert results['processing_time'] < 0.1, f"Chunk size {chunk_size}: Processing too slow"
            
            # Throughput should be reasonable
            assert results['throughput_bps'] > 100000, f"Chunk size {chunk_size}: Low throughput"
    ...
    def test_streaming_data_preparation_performance(self, streaming_manager):
        ...
        for audio_size in audio_sizes:
            ...
            start_time = time.time()
            streaming_data = streaming_manager.prepare_streaming_audio(test_audio, is_cached=False)
            end_time = time.time()
            ...
            assert preparation_time < 1.0, f"Audio size {audio_size}: Preparation too slow ({preparation_time:.3f}s)"
            ...
            assert cached_preparation_time <= preparation_time, "Cached preparation should be faster"
    ...
    def test_concurrent_streaming_operations(self, streaming_manager):
        ...
        for i in range(num_streams):
            thread = threading.Thread(target=lambda i=i: results.append(streaming_worker(i)))
        ...
        assert success_rate > 0.8, f"Concurrent streaming success rate: {success_rate:.2%}"
        
        # Should complete within reasonable time
        assert total_time < 5.0, f"Concurrent streaming took too long: {total_time:.2f}s"
    ...
    def test_audio_format_conversion_performance(self):
        ...
        for audio_size in audio_sizes:
            ...
            for format_type in formats:
                ...
                gradio_uri = AudioFormatConverter.convert_to_gradio_format(test_audio, format_type)
                ...
                assert conversion_time < 0.5, f"Format {format_type}, size {audio_size}: Conversion too slow"
                
                # Test Gradio component data creation
                start_time = time.time()
                component_data = AudioFormatConverter.create_gradio_audio_component_data(test_audio)
                end_time = time.time()
...
class TestNetworkConditionsSimulation:
    ...
    async def test_high_latency_conditions(self, tts_service):
        ...
        with patch('aiohttp.ClientSession', return_value=mock_session):
            ...
            assert 2.0 <= response_time < 5.0  # Should include the delay but not timeout
...
class TestPerformanceOptimization:
    ...
    async def test_cache_hit_performance(self, optimized_service):
        ...
        assert avg_cache_hit_time < 0.01, f"Cache hits too slow: {avg_cache_hit_time:.4f}s"
        
        # Should be consistent
        max_cache_hit_time = max(cache_hit_times)
        assert max_cache_hit_time < 0.05, f"Inconsistent cache performance: {max_cache_hit_time:.4f}s"
    ...
    def test_concurrent_cache_performance(self, optimized_service):
        ...
        if successful_ops:
            avg_write_time = sum(r['write_time'] for r in successful_ops) / len(successful_ops)
            avg_read_time = sum(r['read_time'] for r in successful_ops) / len(successful_ops)
            
            # Cache operations should be fast even under concurrency
            assert avg_write_time < 0.01, f"Concurrent cache writes too slow: {avg_write_time:.4f}s"
            assert avg_read_time < 0.005, f"Concurrent cache reads too slow: {avg_read_time:.4f}s"
        
        # Overall performance should be reasonable
        ops_per_second = len(performance_results) / total_time
        assert ops_per_second > 1000, f"Cache throughput too low: {ops_per_second:.0f} ops/sec"
...
class TestReliabilityUnderAdverseConditions:
    ...
    async def test_sustained_error_conditions(self, reliable_service):
        ...
        assert success_count > 0, "Service should eventually succeed despite sustained errors"
    ...
    def test_resource_exhaustion_handling(self, reliable_service):
        ...
        if 'backends' in cache_stats and 'memory' in cache_stats['backends']:
            ...
            assert entry_count < 100, f"Cache did not enforce limits: {entry_count} entries"
