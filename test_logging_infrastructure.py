...
from src.utils.url_metadata_logger import (
    ...
    log_validation, log_upload, log_retry, LOG_ROTATION_MAX_BYTES, 
...
def assert_valid_uuid(uuid_string):
    ...
    try:
        ...
        assert str(uuid_obj) == uuid_string.lower(), f"UUID string format mismatch: {uuid_string}"
...
class TestURLMetadataMonitor:
    ...
    def test_alert_mechanism(self, mock_datetime):
        ...
        assert len(self.monitor.alert_history) > 0, "Expected alert for high failure rate (25%)"
        
        # Verify alert content structure and values
        alert = self.monitor.alert_history[0]
        
        # Required fields
        required_fields = ['timestamp', 'alert_name', 'threshold_value', 'actual_value', 'window_minutes']
        ...
        assert alert['actual_value'] > alert['threshold_value'], "Actual failure rate should exceed threshold"
        assert abs(alert['actual_value'] - 0.25) < 0.01, f"Expected ~25% failure rate, got {alert['actual_value']}"
    ...
    def test_alert_mechanism_no_false_positives(self, mock_datetime):
        ...
        self.monitor.record_validation("http://bad.com", False, 50.0, error_type="Invalid URL")
        ...
        assert len(self.monitor.alert_history) == 0, "No alerts should be triggered for 5% failure rate"
    ...
    def test_alert_cooldown_mechanism(self):
        ...
        for i in range(10):
            self.monitor.record_validation(f"http://url{i}.com", True, 50.0)
        for i in range(5):
            self.monitor.record_validation(f"http://bad{i}.com", False, 50.0, error_type="Invalid URL")
        ...
        first_alert_time = datetime.fromisoformat(self.monitor.alert_history[0]['timestamp'])
        
        # Try to trigger another alert immediately (should be blocked by cooldown)
        self.monitor._check_alerts()
        assert len(self.monitor.alert_history) == 1, "Second alert should be blocked by cooldown"
        ...
        with patch('src.monitoring.url_metadata_monitor.datetime') as mock_datetime:
            future_time = first_alert_time + timedelta(minutes=121)
            ...
            assert len(self.monitor.alert_history) == 2, "Second alert should be triggered after cooldown"
...
class TestLoggingIntegration:
    ...
    def test_url_utils_logging(self, mock_head):
        ...
        with patch('src.utils.url_metadata_logger.log_validation') as mock_log:
    ...
    def test_data_collector_logging(self):
        ...
        with patch.object(collector, '_fetch_data') as mock_fetch:
...
class TestPerformanceImpact:
    ...
    def test_logging_overhead(self):
        ...
        assert overhead_ratio < 2.0, f"Logging overhead too high: {overhead_ratio:.2f}x"

        # Optionally, use pytest's capsys or logging for debugging
        # pytest.param.capsys.readouterr()  # If you need to capture output
...
class TestErrorHandling:
    ...
    def test_monitor_handles_invalid_data(self):
        ...
        for url, success, duration in invalid_validation_cases:
            try:
                monitor.record_validation(url, success, duration, error_type="test_error")
            except Exception:
        ...
        for url, success, duration, size in invalid_upload_cases:
            try:
                monitor.record_upload(url, success, duration, size, error_type="test_error")
            except Exception:
        ...
        assert 'timestamp' in summary or len(summary) == 0  # Could be empty if all data was rejected
