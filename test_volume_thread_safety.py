...
def test_volume_parameter_passing():
    ...
    class MockTTSService:
        ...
        async def _synthesize_with_api(self, text: str, voice_id: str, volume: float = None) -> bytes:
            """Mock API method that records the volume used"""
            self.last_effective_volume = volume if volume is not None else self.config.volume
...
def test_concurrent_volume_usage():
    ...
    async def test_concurrent_calls():
        ...
        assert volumes_used == expected_volumes, f"Expected {expected_volumes}, got {volumes_used}"
