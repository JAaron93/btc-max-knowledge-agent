...
def test_buffer_optimization():
    ...
    try:
        ...
        test_cases = [
            ...
            (1024 * 1024, False, 'high_throughput'),  # 1MB - Large file, high throughput
        ]
        for audio_size, is_cached, connection_type in test_cases:
            ...
            if buffer_size > audio_size * 2:  # Buffer shouldn't be more than 2x audio size
                print(f"⚠️  Warning: Buffer size ({buffer_size}) seems large for audio size ({audio_size})")
...
def test_imports():
    ...
    try:
        ...
        assert isinstance(STREAMING_BUFFER_SIZES, dict), f"Expected dict, got {type(STREAMING_BUFFER_SIZES)}"
        assert len(STREAMING_BUFFER_SIZES) > 0, "STREAMING_BUFFER_SIZES should not be empty"
        print(f"  ✓ STREAMING_BUFFER_SIZES accessible ({len(STREAMING_BUFFER_SIZES)} entries)")
        
        # Test AudioFormatConverter class instantiation
        converter = AudioFormatConverter()
        assert hasattr(converter, 'convert'), "AudioFormatConverter should have convert method"
        print("  ✓ AudioFormatConverter instantiable")
        
        # Test AudioStreamProcessor class instantiation
        processor = AudioStreamProcessor()
        assert hasattr(processor, 'process_chunk'), "AudioStreamProcessor should have process_chunk method"
