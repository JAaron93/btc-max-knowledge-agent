...
from utils.tts_service import get_tts_service, TTSConfig
...
def test_streaming_manager():
    ...
    try:
        ...
        synth_data = manager.create_synthesized_audio_data(mock_audio, synthesis_time=1.5)
...
def test_convenience_functions():
    ...
    try:
        ...
        print(f"Gradio tuple: {type(gradio_tuple)}, length: {len(gradio_tuple) if isinstance(gradio_tuple, tuple) else 'not tuple'}")
...
async def test_tts_integration():
    ...
    if not tts_service.is_enabled():
        print("⚠️  TTS service not enabled (missing API key), skipping TTS integration test")
        return True
    ...
    try:
        ...
        streaming_data = streaming_manager.create_synthesized_audio_data(audio_bytes, synthesis_time=2.0)
...
def test_error_handling():
    ...
    try:
        ...
        try:
            ...
            print(f"❌ Should have failed with {len(below_threshold_data)}-byte audio data (below {MIN_AUDIO_DATA_SIZE}-byte minimum)")
            return False
        except AudioProcessingError:
            print(f"✅ Correctly handled {len(below_threshold_data)}-byte audio data (below {MIN_AUDIO_DATA_SIZE}-byte minimum)")
