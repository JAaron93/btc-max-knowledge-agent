...
def demo_single_result_formatting() -> None:
    ...
    try:
        ...
        print("-" * 40)
        result = QueryResultFormatter.format_single_result(SAMPLE_RESULTS[0], include_score=True)
        print(result)
...
def demo_mcp_formatting() -> None:
    ...
    try:
        ...
        mcp_response = MCPResponseFormatter.format_for_mcp(
            results=MCP_SAMPLE_RESPONSES,
            query=query
        )
        
        if not mcp_response or 'content' not in mcp_response or not mcp_response['content']:
...
def demo_assistant_formatting() -> None:
    ...
    try:
        ...
        required_keys = ['answer', 'source_summary', 'formatted_sources', 'structured_sources']
...
def demo_mixed_results_handling() -> None:
    ...
    try:
        ...
        try:
            ...
            if 'summary' in structured:
                print(f"Summary: {truncate_text(structured['summary'], max_length=150)}")
            ...
            try:
                ...
                try:
                    ...
                    assistant_response = AssistantResponseFormatter.format_assistant_response(
                        answer=(
                    ...
                    required_keys = [
                        'answer', 'source_summary', 'formatted_sources',
                        'structured_sources'
                    ]
                    missing_keys = set(required_keys) - set(
                        assistant_response.keys()
                    )
                    if missing_keys:
                        raise ValueError(
            ...
            print("\n" + "=" * 60)
            print("✅ Mixed Results Handling Demo Completed"
                  " (with error handling)")
            print("=" * 60)
        ...
        try:
            mcp_formatted = MCPResponseFormatter.format_for_mcp(
                mixed_results, 
                "Bitcoin fundamentals and technologies"
            )
