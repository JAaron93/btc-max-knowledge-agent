...
class TestPublicAPICacheHit:
    ...
    def test_cache_hit_basic(self):
        ...
        with patch('src.utils.url_utils.validate_url_format', side_effect=mock_validate_url_format) as mock_validate:
            ...
            assert call_count == 1, f"Expected validate_url_format to only be called once due to caching, but it was called {call_count} times"
    ...
    def test_cache_hit_with_invalid_url(self):
        ...
        with patch('src.utils.url_utils.validate_url_format', side_effect=mock_validate_url_format) as mock_validate:
            ...
            assert result1[test_url]['valid'] == result2[test_url]['valid'], "valid field should be consistent"
            assert result1[test_url]['secure'] == result2[test_url]['secure'], "secure field should be consistent"
    ...
    def test_cache_hit_multiple_urls(self):
        ...
        with patch('src.utils.url_utils.validate_url_format', side_effect=mock_validate_url_format) as mock_validate:
            ...
            assert call_count >= 3, f"Expected at least 3 calls to validate_url_format, got {call_count}"
            ...
            assert call_count == 3, f"Expected validate_url_format to only be called 3 times total due to caching, but it was called {call_count} times"
            
            # Verify core validation results are consistent
            for url in test_urls:
                assert result1[url]['valid'] == result2[url]['valid'], f"valid field should be consistent for {url}"
                assert result1[url]['secure'] == result2[url]['secure'], f"secure field should be consistent for {url}"
