...
def test_streaming_status_endpoint():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"❌ Streaming status failed: {response.status_code} - {response.text}")
            return False
...
def test_streaming_test_endpoint():
    ...
    try:
        ...
        response = requests.post(
            f"{API_BASE_URL}/tts/streaming/test",
            json=payload,
            timeout=30
        )
        ...
        if response.status_code == 200:
            ...
            print("⚠️  TTS service not enabled (likely missing API key), skipping streaming test")
            return True
        else:
...
def test_query_with_streaming():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"❌ Query with streaming failed: {response.status_code} - {response.text}")
            return False
