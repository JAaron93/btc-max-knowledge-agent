...
from utils.multi_tier_audio_cache import (CacheConfig, MemoryCacheBackend,
                                          MultiTierAudioCache,
                                          SQLiteCacheBackend)
from utils.tts_service import TTSConfig, TTSService
...
def test_cache_backends():
    ...
    print(
        f"   Stats: {stats['entry_count']} entries, {stats['total_size_bytes']} bytes"
    )
...
def test_multi_tier_cache():
    ...
    for i, text in enumerate(test_texts):
        ...
        print(
            f"   Retrieved text {i+1}: {'SUCCESS' if success else 'FAILED'} ({(end_time - start_time)*1000:.2f}ms)"
        )
    ...
    for tier in ["memory", "persistent", "distributed"]:
        ...
        print(
            f"     {tier.capitalize()}: {hits} hits, {misses} misses, {hit_rate:.2%} hit rate"
        )
