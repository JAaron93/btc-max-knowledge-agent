...
def demo_session_security():
    ...
    try:
        ...
        # If reset method fails, create fresh instance
        rate_limiter = SessionRateLimiter()
    ...
    print(
        "   • [2024-01-01 10:00:02] Ownership validation failed: sess_abc123 != sess_def456"
    )
    print(
        "   • [2024-01-01 10:00:03] Session access granted: sess_abc123 from 192.168.1.100"
    )
    ...
    try:
        ...
        for endpoint, stat in stats.items():
            ...
            print(
                f"   • {endpoint_name:15s}: {clients} clients, "
    ...
    try:
        ...
        print(
            f"   • Average Session Age: {session_stats['average_session_age_minutes']:.1f} minutes"
        )
    except Exception as e:
        print(f"   ❌ Error retrieving session stats: {str(e)}")
        print("   • Session manager may not be properly initialized")
