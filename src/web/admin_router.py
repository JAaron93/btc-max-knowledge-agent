...
from .admin_auth import verify_admin_access, get_admin_authenticator
...
async def force_delete_session(session_id: str):
    ...
    try:
        ...
        logger.error(f"Failed to force delete session {session_id[:8]}... for admin: {str(e)}")
...
async def get_admin_auth_stats():
    ...
    try:
        ...
        return {
            "admin_auth_stats": stats,
            "timestamp": time.time()
        }
    except Exception as e:
        logger.error(f"Failed to get admin auth stats: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Failed to get admin auth stats: {str(e)}")
...
async def admin_health_check():
    ...
    try:
        ...
        return {
            ...
            "rate_limiter_clients": sum(stat["active_clients"] for stat in rate_stats.values()),
            "timestamp": time.time(),
            "admin_access": True
        }
    except Exception as e:
