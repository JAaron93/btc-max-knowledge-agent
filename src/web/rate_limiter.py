    ...
    def is_allowed(self, client_id: str) -> bool:
        ...
        with self._lock:
            ...
            if len(client_requests) < self.max_requests:
                ...
                logger.warning(f"Rate limit exceeded for client {client_id}: {len(client_requests)} requests in {self.window_seconds}s")
                return False
...
class SessionRateLimiter:
    ...
    def __init__(self):
        ...
        self.session_info_limiter = RateLimiter(max_requests=20, window_seconds=60)  # 20 per minute
        self.session_delete_limiter = RateLimiter(max_requests=5, window_seconds=60)  # 5 per minute
        self.session_create_limiter = RateLimiter(max_requests=10, window_seconds=60)  # 10 per minute
