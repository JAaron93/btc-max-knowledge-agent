...
class SessionData:
    ...
    def __init__(self, session_id: str, max_history_turns: int = 50):
        ...
        self.tts_preferences = {"enabled": True, "volume": 0.7}
    ...
    def add_conversation_turn(
        self, question: str, answer: str, sources: List[Dict] = None
    ):
        ...
        if len(self.conversation_history) > self.max_history_turns:
            ...
            logger.info(
                f"Trimmed conversation history for session {self.session_id[:8]}... to {self.max_history_turns} turns"
            )
    ...
    def get_conversation_context(self, max_turns: int = 5) -> List[Dict[str, Any]]:
        ...
        return (
            self.conversation_history[-max_turns:] if self.conversation_history else []
        )
    ...
    def get_memory_usage(self) -> Dict[str, Any]:
        ...
        Uses pympler.asizeof if available for most accurate measurement,
        otherwise falls back to recursive sys.getsizeof which provides
        better accuracy than JSON serialization by including Python object overhead.
        """
        try:
            ...
            # Fallback to recursive sys.getsizeof for more accurate memory estimation
            history_size = self._calculate_recursive_size(self.conversation_history)
    ...
    def _calculate_recursive_size(self, obj, seen=None) -> int:
        """
        Recursively calculate the memory size of an object using sys.getsizeof.

        This provides a more accurate estimation than JSON serialization by:
        - Traversing nested objects (dicts, lists, tuples, sets)
        - Avoiding double-counting of shared objects using a seen set
        - Including Python object overhead that JSON serialization misses

        Args:
            obj: The object to measure
            seen: Set of object IDs already measured (prevents infinite recursion)
        ...
        if isinstance(obj, dict):
            for key, value in obj.items():
                size += self._calculate_recursive_size(key, seen)
                size += self._calculate_recursive_size(value, seen)
        elif isinstance(obj, (list, tuple, set, frozenset)):
            for item in obj:
                size += self._calculate_recursive_size(item, seen)
        elif hasattr(obj, "__dict__"):
            # Handle custom objects with attributes
            size += self._calculate_recursive_size(obj.__dict__, seen)
        elif hasattr(obj, "__slots__"):
            # Handle objects with __slots__
            for slot in obj.__slots__:
                if hasattr(obj, slot):
                    size += self._calculate_recursive_size(getattr(obj, slot), seen)
...
class SessionManager:
    ...
    def __init__(
        ...
        self.session_timeout_minutes = session_timeout_minutes or int(
            os.getenv("SESSION_TIMEOUT_MINUTES", "60")
        )
        self.cleanup_interval_minutes = cleanup_interval_minutes or int(
            os.getenv("SESSION_CLEANUP_INTERVAL_MINUTES", "10")
        )
        self.max_history_turns = max_history_turns or int(
            os.getenv("SESSION_MAX_HISTORY_TURNS", "50")
        )

        # Session ID configuration
        self.session_id_length = session_id_length or int(
            os.getenv("SESSION_ID_LENGTH", "32")
        )
        self.session_id_charset = session_id_charset or os.getenv(
            "SESSION_ID_CHARSET", "0123456789abcdef"
        )

        # Validate session ID configuration
        if self.session_id_length < 16:
            raise ValueError(
                "Session ID length must be at least 16 characters for security"
            )
        ...
        logger.info(
            f"SessionManager initialized with {self.session_timeout_minutes}min timeout, "
            f"{self.max_history_turns} max turns per session, "
            f"{self.session_id_length}-char session IDs using charset size: {len(self.session_id_charset)}"
        )
    ...
    def create_session(self) -> tuple[str, SessionData]:
        ...
        with self._lock:
            ...
            while session_id in self.sessions:
                collision_count += 1
                logger.warning(
                    f"Session ID collision detected (attempt {collision_count}), regenerating..."
                )
                session_id = self._generate_session_id()
            ...
            logger.info(
                f"Created new session: {session_id[:8]}... (length: {len(session_id)})"
            )
    ...
    def _generate_session_id(self) -> str:
        ...
        random_bytes = secrets.token_hex(
            16
        )  # 16 bytes of cryptographically secure random data
        ...
        if self.session_id_charset == "0123456789abcdef":
            ...
            session_id = self._convert_to_charset(
                full_hash, self.session_id_charset, self.session_id_length
            )

        return session_id

    def _convert_to_charset(
        self, hex_string: str, charset: str, target_length: int
    ) -> str:
        """
    ...
    def _start_cleanup_timer(self):
        ...
        self._cleanup_timer = threading.Timer(
            self.cleanup_interval_minutes * 60, self._background_cleanup
        )
    ...
    def get_session_id_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "example_pattern": (
                ...
                else "".join([self.session_id_charset[0]] * self.session_id_length)
            ),
            "entropy_bits": round(
                self.session_id_length * math.log2(len(self.session_id_charset)), 2
            ),
        }
