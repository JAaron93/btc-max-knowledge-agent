...
class TTSServerError(TTSError):
    ...
    def __init__(self, message: str, status_code: int):
        super().__init__(
            message, error_code=f"SERVER_ERROR_{status_code}", recoverable=True
        )
...
class TTSCircuitOpenError(TTSError):
    ...
    def __init__(
        self,
        message: str = "Circuit breaker is open - requests are being short-circuited",
    ):
...
class TTSErrorHandler:
    ...
    def _classify_http_error(self, status_code: int, response_text: str) -> TTSError:
        ...
        if status_code == 401:
            ...
            return TTSServerError(
                f"Server error ({status_code}): {response_text}", status_code
            )
        else:
            return TTSError(
    ...
    async def execute_with_retry(
        ...
        if not self.circuit_breaker.can_execute():
            ...
            error_msg = (
                f"Circuit breaker is {circuit_state['state']} - "
        ...
        for attempt in range(
            max(
                self.retry_config["max_retries_429"],
                self.retry_config["max_retries_5xx"],
            )
            + 1
        ):
            try:
                ...
                if attempt < self.retry_config["max_retries_5xx"]:
                    ...
                    self._log_retry_attempt(
                        attempt, self.retry_config["max_retries_5xx"], last_error, delay
                    )
                    await asyncio.sleep(delay)
                    continue
                else:
                    logger.error(
                        f"Timeout retries exhausted after {attempt + 1} attempts"
                    )
                    break
                ...
                if attempt < self.retry_config["max_retries_5xx"]:
                    ...
                    logger.error(
                        f"Network error retries exhausted after {attempt + 1} attempts"
                    )
                    break
                ...
                if e.status == 429:  # Rate limit - use 429-specific retry config
                    ...
                    if attempt < self.retry_config["max_retries_429"]:
                        ...
                        logger.error(
                            f"Rate limit retries exhausted after {attempt + 1} attempts"
                        )
                        break

                elif (
                    500 <= e.status < 600
                ):  # Server errors - use 5xx-specific retry config
                    ...
                    if attempt < self.retry_config["max_retries_5xx"]:
                        ...
                        logger.error(
                            f"Server error retries exhausted after {attempt + 1} attempts"
                        )
                        break
                    ...
                    logger.error(
                        "API key error detected - no retries will be attempted"
                    )
                    break
                    ...
                    logger.error(
                        f"Non-retryable HTTP error {e.status} - no retries will be attempted"
                    )
                    break
                ...
                logger.error(
                    f"Unexpected error during TTS operation: {str(e)} - no retries will be attempted"
                )
                break
        ...
        if isinstance(last_error, TTSAPIKeyError):
            ...
            logger.error(
                f"TTS operation failed after {total_attempts} attempts - falling back to muted state"
            )
