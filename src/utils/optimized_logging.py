...
class OptimizedURLMetadataLogger:
    ...
    def __init__(self, log_level: str = "INFO"):
        ...
        self.validation_logger = PerformanceOptimizedLogger("url_metadata.validation", log_level)
        self.upload_logger = PerformanceOptimizedLogger("url_metadata.upload", log_level)
        self.retrieval_logger = PerformanceOptimizedLogger("url_metadata.retrieval", log_level)
        self.sanitization_logger = PerformanceOptimizedLogger("url_metadata.sanitization", log_level)
        self.retry_logger = PerformanceOptimizedLogger("url_metadata.retry", log_level)
        self.metrics_logger = PerformanceOptimizedLogger("url_metadata.metrics", log_level)
    ...
    def log_sanitization(self, original_url: str, sanitized_url: str, 
                        changes_made: list, duration_ms: Optional[float] = None):
        ...
        def wrapper(*args, **kwargs):
            if logger.is_debug_enabled():
                ...
                try:
                    ...
                    duration_ms = (time.time() - start_time) * 1000
                    logger.error(f"{operation_name} failed after {duration_ms:.1f}ms: {e}")
                    raise
            else:
...
def log_validation_optimized(url: str, is_valid: bool, validation_type: str, **kwargs):
    ...
    optimized_url_metadata_logger.log_validation(url, is_valid, validation_type, **kwargs)
...
def log_retry_optimized(operation: str, attempt: int, max_attempts: int, error: str, **kwargs):
    """Optimized retry logging function."""
    optimized_url_metadata_logger.log_retry(operation, attempt, max_attempts, error, **kwargs)
...
def configure_optimized_logging():
    ...
    if os.getenv("ENVIRONMENT") == "production":
        ...
        for logger_name in ["url_metadata.validation", "url_metadata.upload", 
                           "url_metadata.retrieval", "url_metadata.sanitization"]:
