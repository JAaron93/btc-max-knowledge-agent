...
class CacheConfig:
    ...
    def from_env(cls) -> 'CacheConfig':
        ...
        return cls(
            ...
            cleanup_interval_minutes=int(os.getenv("CACHE_CLEANUP_INTERVAL_MINUTES", "60")),
            enable_cache_warming=os.getenv("CACHE_ENABLE_WARMING", "true").lower() == "true",
            enable_statistics=os.getenv("CACHE_ENABLE_STATISTICS", "true").lower() == "true",
        )
...
class MemoryCacheBackend(BaseCacheBackend):
    ...
    def __init__(self, config: CacheConfig):
        ...
        self.cache = AudioCache(
            ...
            max_memory_mb=config.memory_max_mb
        )
    ...
    def get_stats(self) -> Dict[str, Any]:
        with self._lock:
            ...
            stats.update({
                ...
                'expired_entries': sum(1 for exp_time in self._ttl_map.values() 
    ...
    def cleanup_expired(self) -> int:
        with self._lock:
            expired_keys = [
                key for key, exp_time in self._ttl_map.items()
                if datetime.now() > exp_time
            ]
    ...
    def _init_db(self):
        ...
        with self._lock:
            ...
            try:
                ...
                """)
                conn.execute("CREATE INDEX IF NOT EXISTS idx_expires_at ON audio_cache(expires_at)")
                conn.execute("CREATE INDEX IF NOT EXISTS idx_accessed_at ON audio_cache(accessed_at)")
                conn.commit()
            finally:
                conn.close()
    ...
    def _init_backends(self):
        ...
        try:
            ...
            if self.config.backend in ['sqlite', 'multi-tier']:
                ...
                logger.warning("Diskcache backend not implemented, falling back to SQLite")
                self.backends['persistent'] = SQLiteCacheBackend(self.config)
        ...
        if 'persistent' in self.backends:
            try:
                ...
                if data:
                    ...
                    # Warm memory cache
                    if 'memory' in self.backends:
                        self.backends['memory'].put(key, data, self.config.ttl_hours * 3600)
                    return data
                else:
    ...
    def cleanup_expired(self) -> Dict[str, int]:
        ...
        for tier_name, backend in self.backends.items():
            try:
                ...
                if count > 0:
                    logger.info(f"Cleaned up {count} expired entries from {tier_name} cache")
            except Exception as e:
