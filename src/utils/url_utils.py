...
def normalize_url(url: str) -> Optional[str]:
    """Backward compatibility alias for normalize_url_rfc3986."""
    return normalize_url_rfc3986(url)


def normalize_url_format(url: str) -> Optional[str]:
    """Alternative alias for normalize_url_rfc3986."""
    return normalize_url_rfc3986(url)
...
def is_private_ip(url: str) -> bool:
    ...
    """
    # For backward compatibility, return the inverse of is_secure_url
    # since private IPs are considered insecure
    return not is_secure_url(url)
...
def is_url_valid(url: str) -> bool:
    """
    Check if a URL is valid and secure.

    This function checks both URL format and security.
    For more detailed validation, use the URLValidator class.

    Args:
        url: The URL to validate

    Returns:
        bool: True if URL is valid and secure, False otherwise
    """
    return validate_url_format(url) and is_secure_url(url)
...
def is_secure_url(url: str) -> bool:
    ...
    if url_clean != url:
        ...
        record_validation(
            url, False, duration_ms, error_type="contains_control_characters"
        )
        return False
    ...
    try:
        ...
        try:
            ...
            # Check if this is a loopback IP first (127.0.0.1, ::1)
            if ip.is_loopback:
                if not Config.ALLOW_LOCALHOST_URLS:
                    validation_details["error"] = f"localhost_ip_not_allowed: {ip}"
                    duration_ms = (time.time() - start_time) * 1000
                    log_validation(
                        url,
                        False,
                ...
                if ip.is_unspecified or ip.is_multicast or ip.is_reserved:
                    ...
                    record_validation(
                        url, False, duration_ms, error_type="unspecified_or_reserved_ip"
                    )
                    return False
        except ValueError:
            ...
            if parsed.hostname.lower() in ["localhost", "127.0.0.1", "::1"]:
                if not Config.ALLOW_LOCALHOST_URLS:
                    ...
                    record_validation(
                        url, False, duration_ms, error_type="localhost_not_allowed"
                    )
                    return False
        ...
        if dangerous_found:
            ...
            record_validation(
                url, False, duration_ms, error_type="encoded_dangerous_scheme"
            )
            return False
...
def normalize_url_rfc3986(url: str) -> Optional[str]:
    ...
    if not (
        ...
        or re.match(PROTOCOL_PATTERN, url)
        or re.match(SIMPLE_DOMAIN_PATTERN, url)
        or re.match(COMPLEX_DOMAIN_PATTERN, url)
    ):
        return None
    ...
    try:
        ...
        for char in url:
            ...
            if (32 <= ord(char) <= 126) or (
                ord(char) >= 128 and char not in special_chars
            ):
        ...
        duration_ms = (time.time() - start_time) * 1000
        log_sanitization(
            original_url, normalized, changes_made, duration_ms=duration_ms
        )
...
def validate_url_batch(
    urls: List[str], check_accessibility: bool = False, max_workers: int = 10
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {executor.submit(validate_single_url, url): url for url in urls}
...
def _cache_validation(url: str, is_valid: bool) -> None:
    ...
    with _cache_lock:
        ...
        if len(_validation_cache) > 10000:
            ...
            sorted_items = sorted(_validation_cache.items(), key=lambda x: x[1][1])
...
def validate_url_format(url: str) -> bool:
    ...
    if "@" in url_lower and "://" not in url_lower and not url_lower.startswith("//"):
        ...
        if "." in domain_part and not domain_part.startswith(
            "."
        ):  # Has a dot and not starting with dot
    ...
    try:
        ...
        if not parsed.scheme or not parsed.netloc:
            ...
            if (
                ...
                and not cleaned_url.startswith("//")
            ):
    ...
    if not _use_session:
        try:
            ...
            if not is_accessible:
                record_validation(
                    url, False, duration_ms, error_type=f"http_{response.status_code}"
                )
            else:
                record_validation(url, True, duration_ms)
...
def validate_and_sanitize_url(
    url: str, check_accessibility: bool = False
...
def check_urls_accessibility_parallel(
    urls: List[str], timeout: int = 5, max_workers: int = 10
) -> Dict[str, bool]:
    """
