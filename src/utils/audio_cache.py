...
class AudioCache:
    """
    In-memory LRU cache for storing generated audio data.
    ...
    def __init__(self, max_size: int = 100, max_memory_mb: int = 50):
        ...
        logger.info(
            f"AudioCache initialized with max_size={max_size}, max_memory_mb={max_memory_mb}"
        )
    ...
    def put(self, text: str, audio_data: bytes) -> str:
        ...
        logger.debug(
            f"Cached audio: {text_hash[:8]}... (size: {entry.size_bytes} bytes)"
        )
    ...
    def _evict_oldest(self) -> None:
        ...
        logger.debug(
            f"Evicted cache entry: {oldest_key[:8]}... (size: {oldest_entry.size_bytes} bytes)"
        )
    ...
    def get_by_hash(self, text_hash: str) -> Optional[bytes]:
        """
        Retrieve audio data from cache by hash.
        ...
        logger.debug(
            f"Cache hit: {text_hash[:8]}... (access_count: {entry.access_count})"
        )
    ...
    def remove_by_hash(self, text_hash: str) -> bool:
        """
        Remove entry from cache by hash.
