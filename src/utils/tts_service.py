...
class ConnectionPool:
    ...
    def __init__(self, max_connections: int = 10, connection_timeout: float = 10.0, 
                 read_timeout: float = 30.0, total_timeout: float = 45.0,
        ...
        self._cleanup_interval = cleanup_interval  # Configurable cleanup interval in seconds
    ...
    async def get_session(self) -> aiohttp.ClientSession:
        ...
        async with self._lock:
            ...
            if self._session is None or self._session.closed:
                ...
                logger.debug(f"Created new connection pool with {self.max_connections} max connections")
            ...
            if cache_instance:
                try:
                    ...
                    if isinstance(expired_count, dict):
                        ...
                        try:
                            cleanup_results['cache_entries_removed'] = sum(expired_count.values())
                        except (TypeError, ValueError) as e:
                            logger.warning(f"Invalid dict values in cleanup_expired result: {e}")
                        ...
                        logger.debug("cleanup_expired returned None, assuming 0 entries removed")
                        cleanup_results['cache_entries_removed'] = 0
                    else:
                        # Handle unexpected types
                        logger.warning(f"Unexpected return type from cleanup_expired: {type(expired_count)}. "
...
class TTSService:
    ...
    def __init__(self, config: Optional[TTSConfig] = None, cache_config: Optional[CacheConfig] = None):
        ...
        if not self.config.api_key:
            ...
            logger.info("TTS service initialized with connection pooling and memory management")
    ...
    async def synthesize_text(self, text: str, voice_id: Optional[str] = None, volume: Optional[float] = None) -> bytes:
        ...
        if not self.config.enabled or not self.config.api_key:
            logger.warning("TTS synthesis attempted but service is disabled or API key is missing")
            raise TTSAPIKeyError("TTS service is not enabled or API key is missing")
        
        # Log synthesis request details
        logger.info(f"TTS synthesis request: {len(text)} characters, voice: {voice_id or self.config.voice_id}")
        ...
        if cached_audio:
            ...
            logger.info(f"TTS cache hit: returning {len(cached_audio)} bytes of cached audio")
            return cached_audio
        ...
        try:
            ...
            with self._stats_lock:
                ...
                if len(self._performance_stats['synthesis_times']) > 100:
                    self._performance_stats['synthesis_times'] = self._performance_stats['synthesis_times'][-100:]
            
            logger.info(f"Successfully synthesized {len(audio_data)} bytes of audio in {synthesis_time:.2f}s (cached as {cache_key[:8]}...)")
            return audio_data
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        with self._stats_lock:
            ...
            cache_hit_rate = self._performance_stats['cache_hits'] / max(total_requests, 1)
