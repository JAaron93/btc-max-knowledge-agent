...
class BitcoinDataCollector:
    ...
    def collect_from_rss(self, max_articles: int = 20) -> List[Dict[str, Any]]:
        ...
        with correlation_context() as correlation_id:
            self.validation_logger.info("Starting RSS collection", extra={
        ...
        for feed_url in self.rss_feeds:
            with correlation_context() as feed_correlation_id:
                try:
                    ...
                    self.validation_logger.info("Fetching RSS feed", extra={
                    ...
                    for entry in feed.entries[:max_articles]:
                        try:
                            ...
                            self.validation_logger.info("Processing article", extra={
                                'correlation_id': feed_correlation_id,
                            ...
                            if len(article.text) > 500:  # Only include substantial articles
                                ...
                                self.validation_logger.info("Article extracted successfully", extra={
                                    'correlation_id': feed_correlation_id,
                                    'article_url': entry.link,
                                ...
                                self.validation_logger.warning("Article too short", extra={
                                    'correlation_id': feed_correlation_id,
                                    'article_url': entry.link,
                                    'content_length': len(article.text)
                                })
                            ...
                            self.validation_logger.error("Failed to process article", extra={
                                'correlation_id': feed_correlation_id,
                                'article_url': entry.link,
                                'error': str(e)
                            })
                    ...
                    self.validation_logger.error("Failed to fetch RSS feed", extra={
                        'correlation_id': feed_correlation_id,
                        'feed_url': feed_url,
                        'error': str(e)
                    })
        ...
        self.metrics_logger.info("RSS collection completed", extra={
    ...
    def collect_bitcoin_basics(self) -> List[Dict[str, Any]]:
        ...
        with correlation_context() as correlation_id:
            self.validation_logger.info("Collecting Bitcoin basics", extra={
                'correlation_id': correlation_id
            })
                ...
                try:
                    ...
                    if not sanitized_url:
                        ...
                        if not sanitized_url:
                            ...
                            sanitized_url = FallbackURLStrategy.domain_only_url(primary_url)
                            self.validation_logger.warning(
                                "Using domain-only fallback URL", extra={
                                    'correlation_id': correlation_id,
                    ...
                    self.validation_logger.info("Added document with validated URL", extra={
                        'correlation_id': correlation_id,
                    ...
                    self.validation_logger.error("Failed to process resource", extra={
                        'correlation_id': correlation_id,
            ...
            self.metrics_logger.info("Bitcoin basics collection completed", extra={
                'correlation_id': correlation_id,
    ...
    def collect_genius_act_info(self) -> List[Dict[str, Any]]:
        ...
        with correlation_context() as correlation_id:
            self.validation_logger.info("Collecting GENIUS Act information", extra={
                'correlation_id': correlation_id
            })
                ...
                try:
                    ...
                    if not sanitized_url:
                        ...
                        for fallback_url in gov_urls:
                            ...
                            if sanitized_url:
                                self.validation_logger.info(
                                    "Using government fallback URL", extra={
                                        'correlation_id': correlation_id,
                                        'document_id': resource['id'],
                                        'fallback_url': sanitized_url
                                    }
                        ...
                        if not sanitized_url:
                            ...
                            self.validation_logger.warning(
                                "Using congress.gov domain fallback", extra={
                                    'correlation_id': correlation_id,
                                    'document_id': resource['id'],
                                    'fallback_url': sanitized_url
                                }
                    ...
                    self.validation_logger.info(
                        "Added legislative document with URL", extra={
                            'correlation_id': correlation_id,
                            'document_id': document['id'],
                            'url': document['url'],
                            'category': document['category'],
                    ...
                    self.validation_logger.error(
                        "Failed to process legislative resource", extra={
                            'correlation_id': correlation_id,
                            'resource_id': resource.get('id', 'unknown'),
                            'error': str(e)
                        }
            ...
            self.metrics_logger.info("GENIUS Act collection completed", extra={
                'correlation_id': correlation_id,
                'document_count': len(documents),
                'documents_with_urls': sum(1 for d in documents if d.get('url'))
            })
    ...
    def collect_dapp_information(self) -> List[Dict[str, Any]]:
        ...
        with correlation_context() as correlation_id:
            self.validation_logger.info("Collecting dApp information", extra={
                'correlation_id': correlation_id
            })
                ...
                try:
                    ...
                    if not sanitized_url:
                        ...
                        for fallback_url in doc_urls:
                            ...
                            if sanitized_url:
                                self.validation_logger.info(
                                    "Using documentation fallback URL", extra={
                                        'correlation_id': correlation_id,
                                        'document_id': resource['id'],
                                        'fallback_url': sanitized_url
                                    }
                        ...
                        if not sanitized_url:
                            ...
                            if doc_urls:
                                ...
                                if domain:
                                    ...
                                    self.validation_logger.warning(
                                        "Using domain fallback for dApp", extra={
                                            'correlation_id': correlation_id,
                                            'document_id': resource['id'],
                                            'fallback_url': sanitized_url
                                        }
                    ...
                    self.validation_logger.info(
                        "Added dApp document with URL", extra={
                            'correlation_id': correlation_id,
                            'document_id': document['id'],
                            'url': document['url'],
                            'category': document['category'],
                            'doc_url_count': len(document.get('documentation_urls', []))
                        }
                    )
                    
                except Exception as e:
                    self.validation_logger.error(
                        "Failed to process dApp resource", extra={
                            'correlation_id': correlation_id,
                            'resource_id': resource.get('id', 'unknown'),
                            'error': str(e)
                        }
            ...
            self.metrics_logger.info("dApp collection completed", extra={
                'correlation_id': correlation_id,
                'document_count': len(documents),
                'documents_with_urls': sum(1 for d in documents if d.get('url'))
            })
    ...
    def validate_document_urls(self, documents: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        with correlation_context() as correlation_id:
            self.validation_logger.info("Starting batch URL validation", extra={
                'correlation_id': correlation_id,
                'document_count': len(documents)
            })
            ...
            self.validation_logger.info("Validating URLs in batch", extra={
                'correlation_id': correlation_id,
            ...
            for doc in documents:
                ...
                if doc.get('url'):
                    ...
                    if result.get('valid') and result.get('normalized'):
                        ...
                        if fallback_url:
                            ...
                            self.validation_logger.warning(
                                "Used fallback URL for document", extra={
                                    'correlation_id': correlation_id,
                ...
                # Validate URL arrays
                for url_field in ['canonical_urls', 'government_urls',
                                  'documentation_urls']:
                    if url_field in doc:
                        validated_urls = []
                ...
                validated_doc = GracefulDegradation.null_safe_metadata(
                    validated_doc
                )
            ...
            self.metrics_logger.info("Batch URL validation completed", extra={
                'correlation_id': correlation_id,
    ...
    def collect_all_documents(self, max_news_articles: int = 30) -> List[Dict[str, Any]]:
        ...
        with correlation_context() as main_correlation_id:
            self.validation_logger.info("Starting document collection", extra={
            ...
            try:
                ...
                self.validation_logger.error("Failed to collect Bitcoin basics", extra={
                    'correlation_id': main_correlation_id,
                    'error': str(e)
                })
            ...
            try:
                ...
                self.validation_logger.error("Failed to collect GENIUS Act info", extra={
                    'correlation_id': main_correlation_id,
                    'error': str(e)
                })
            ...
            try:
                ...
                self.validation_logger.error("Failed to collect dApp information", extra={
                    'correlation_id': main_correlation_id,
                    'error': str(e)
                })
            ...
            try:
                ...
                self.validation_logger.error("Failed to collect news articles", extra={
                    'correlation_id': main_correlation_id,
                    'error': str(e)
                })
            ...
            self.metrics_logger.info("Document collection completed", extra={
                'correlation_id': main_correlation_id,
    ...
    def save_documents(self, documents: List[Dict[str, Any]], filename: str = "bitcoin_documents.json"):
        ...
        self.validation_logger.info("Documents saved successfully", extra={
            'filename': filename,
            'document_count': len(documents),
            'filepath': filepath
        })
