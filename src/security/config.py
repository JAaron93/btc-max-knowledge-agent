...
class SecurityConfigurationManager(IConfigurationValidator):
    ...
    def _classify_config_error(
        self, error_message: str
    ) -> tuple[str, SecuritySeverity, float, SecurityAction]:
        """
        ...
        # Resource threshold errors (medium-high severity)
            ...
            keyword in error_lower
            for keyword in ["cpu_threshold", "memory_threshold", "disk_threshold"]
        ):
            return (
    ...
    def validate_config(self, config: SecurityConfiguration) -> ValidationResult:
        ...
        for error in errors:
            ...
            violation_type, severity, confidence_score, recommended_action = (
                self._classify_config_error(error)
            )
        ...
        if violations:
            ...
            _, _, _, recommended_action = self._classify_config_error(
        ...
        overall_confidence = (
            1.0
            if is_valid
            else (sum(v.confidence_score for v in violations) / len(violations))
        )
    ...
    def _safe_int_conversion(self, value: str, default: int, var_name: str) -> int:
        ...
        try:
            ...
            logger.warning(
                f"Invalid integer value for {var_name}: '{value}'. Using default: {default}. Error: {e}"
            )
            return default

    def _safe_float_conversion(
        self, value: str, default: float, var_name: str
    ) -> float:
        """
        ...
        try:
            ...
            logger.warning(
                f"Invalid float value for {var_name}: '{value}'. Using default: {default}. Error: {e}"
            )
            return default
    ...
    def _load_alert_thresholds(self, env_vars: dict) -> dict:
        ...
        return {
            "error_rate_percent": self._safe_float_conversion(
                env_vars.get("ALERT_ERROR_RATE_PERCENT", "10.0"),
                10.0,
                "ALERT_ERROR_RATE_PERCENT",
            ),
            "response_time_seconds": self._safe_float_conversion(
                env_vars.get("ALERT_RESPONSE_TIME_SECONDS", "5.0"),
                5.0,
                "ALERT_RESPONSE_TIME_SECONDS",
            ),
            "memory_usage_percent": self._safe_float_conversion(
                env_vars.get("ALERT_MEMORY_USAGE_PERCENT", "80.0"),
                80.0,
                "ALERT_MEMORY_USAGE_PERCENT",
            ),
            "failed_auth_attempts": self._safe_int_conversion(
                env_vars.get("ALERT_FAILED_AUTH_ATTEMPTS", "5"),
                5,
                "ALERT_FAILED_AUTH_ATTEMPTS",
            ),
            "suspicious_ip_count": self._safe_int_conversion(
                env_vars.get("ALERT_SUSPICIOUS_IP_COUNT", "10"),
                10,
                "ALERT_SUSPICIOUS_IP_COUNT",
            ),
            "query_volume_increase_percent": self._safe_float_conversion(
                env_vars.get("ALERT_QUERY_VOLUME_INCREASE_PERCENT", "300.0"),
                300.0,
                "ALERT_QUERY_VOLUME_INCREASE_PERCENT",
            ),
            "entropy_threshold": self._safe_float_conversion(
                env_vars.get("ALERT_ENTROPY_THRESHOLD", "4.5"),
                4.5,
                "ALERT_ENTROPY_THRESHOLD",
            ),
        }
    ...
    def load_secure_config(self) -> SecurityConfiguration:
        ...
        config = SecurityConfiguration(
            ...
            rate_limit_per_minute=self._safe_int_conversion(
                env_vars.get("RATE_LIMIT_PER_MINUTE", "100"),
                100,
            ...
            rate_limit_refill_rate=self._safe_float_conversion(
                env_vars.get("RATE_LIMIT_REFILL_RATE", "1.67"),
                1.67,
                "RATE_LIMIT_REFILL_RATE",
            ),
            auth_cache_validation_timeout_ms=self._safe_int_conversion(
                env_vars.get("AUTH_CACHE_VALIDATION_TIMEOUT_MS", "100"),
                100,
                "AUTH_CACHE_VALIDATION_TIMEOUT_MS",
            ),
            auth_remote_fetch_timeout_ms=self._safe_int_conversion(
                env_vars.get("AUTH_REMOTE_FETCH_TIMEOUT_MS", "300"),
                300,
                "AUTH_REMOTE_FETCH_TIMEOUT_MS",
            ),
            ...
            injection_detection_threshold=self._safe_float_conversion(
                env_vars.get("INJECTION_DETECTION_THRESHOLD", "0.8"),
                0.8,
                "INJECTION_DETECTION_THRESHOLD",
            ),
            injection_accuracy_target=self._safe_float_conversion(
                env_vars.get("INJECTION_ACCURACY_TARGET", "0.95"),
                0.95,
                "INJECTION_ACCURACY_TARGET",
            ),
            sanitization_confidence_threshold=self._safe_float_conversion(
                env_vars.get("SANITIZATION_CONFIDENCE_THRESHOLD", "0.7"),
                0.7,
                "SANITIZATION_CONFIDENCE_THRESHOLD",
            ),
            # Resource limits
            ...
            max_concurrent_system=self._safe_int_conversion(
                env_vars.get("MAX_CONCURRENT_SYSTEM", "200"),
                200,
                "MAX_CONCURRENT_SYSTEM",
            ),
            processing_timeout_seconds=self._safe_int_conversion(
                env_vars.get("PROCESSING_TIMEOUT_SECONDS", "60"),
                60,
                "PROCESSING_TIMEOUT_SECONDS",
            ),
            estimated_processing_limit_seconds=self._safe_int_conversion(
                env_vars.get("ESTIMATED_PROCESSING_LIMIT_SECONDS", "30"),
                30,
                "ESTIMATED_PROCESSING_LIMIT_SECONDS",
            ),
            cpu_threshold_percent=self._safe_float_conversion(
                env_vars.get("CPU_THRESHOLD_PERCENT", "85.0"),
                85.0,
                "CPU_THRESHOLD_PERCENT",
            ),
            memory_threshold_percent=self._safe_float_conversion(
                env_vars.get("MEMORY_THRESHOLD_PERCENT", "90.0"),
                90.0,
                "MEMORY_THRESHOLD_PERCENT",
            ),
            disk_threshold_percent=self._safe_float_conversion(
                env_vars.get("DISK_THRESHOLD_PERCENT", "95.0"),
                95.0,
                "DISK_THRESHOLD_PERCENT",
            ),
            # Monitoring and alerting
            monitoring_enabled=self._parse_boolean(
                env_vars.get("MONITORING_ENABLED", "true"), True
            ),
            ...
            alert_response_time_seconds=self._safe_int_conversion(
                env_vars.get("ALERT_RESPONSE_TIME_SECONDS", "10"),
                10,
                "ALERT_RESPONSE_TIME_SECONDS",
            ),
            anomaly_detection_time_seconds=self._safe_int_conversion(
                env_vars.get("ANOMALY_DETECTION_TIME_SECONDS", "300"),
                300,
                "ANOMALY_DETECTION_TIME_SECONDS",
            ),
        ...
        if not validation_result.is_valid:
            ...
            raise ValueError(
                f"Invalid security configuration: {'; '.join(error_messages)}"
            )
