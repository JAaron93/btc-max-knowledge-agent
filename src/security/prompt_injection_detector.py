...
class PromptInjectionDetector(IPromptInjectionDetector):
    ...
    async def detect_injection(self, query: str, context: Dict[str, Any]) -> DetectionResult:
        ...
        for compiled_pattern, pattern_info in self._compiled_patterns:
            if compiled_pattern.search(query):
                ...
                if pattern_info.injection_type == InjectionType.ROLE_CONFUSION and "act as" in query.lower():
                    legitimate_act_as = any(phrase in query.lower() for phrase in [
                        "responsible investor", "professional", "advisor", "consultant", 
                        "teacher", "guide", "helper", "expert in"
                    ])
        ...
        if context_confidence > max_confidence:
            ...
            risk_level = SecuritySeverity.ERROR if context_confidence > 0.8 else SecuritySeverity.WARNING
        ...
        if sophistication_confidence > max_confidence:
            ...
            risk_level = SecuritySeverity.CRITICAL if sophistication_confidence > 0.9 else SecuritySeverity.ERROR
    ...
    async def _detect_sophisticated_attacks(self, query: str) -> float:
        ...
        if re.search(r'[a-zA-Z]{1}[\s\-_\.]{1}[a-zA-Z]{1}', query):  # Character separation
    ...
    def _determine_action(self, confidence: float, risk_level: SecuritySeverity) -> SecurityAction:
    ...
    def validate_query_parameters(self, top_k: int, similarity_threshold: float) -> ValidationResult:
        """
        ...
        if not (self.MIN_SIMILARITY_THRESHOLD <= similarity_threshold <= self.MAX_SIMILARITY_THRESHOLD):
            is_valid = False
            violation = SecurityViolation(
    ...
    def get_detection_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "pattern_types": {
                injection_type.value: sum(1 for p in self._injection_patterns if p.injection_type == injection_type)
