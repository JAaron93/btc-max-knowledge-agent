...
class SecurityValidator(ISecurityValidator):
    ...
    def __init__(self, config: SecurityConfiguration):
        ...
        logger.info(f"SecurityValidator initialized. Library status: {self._get_library_status_summary()}")
    
    def _get_library_status_summary(self) -> str:
        """Get a summary of library availability status."""
        status = []
        status.append(f"libinjection:{'OK' if self.library_health.libinjection_available else 'UNAVAILABLE'}")
        status.append(f"pymodsecurity:{'OK' if self.library_health.pymodsecurity_available else 'UNAVAILABLE'}")
        status.append(f"bleach:{'OK' if self.library_health.bleach_available else 'UNAVAILABLE'}")
        status.append(f"markupsafe:{'OK' if self.library_health.markupsafe_available else 'UNAVAILABLE'}")
        return ", ".join(status)
    ...
    def _validate_utf8_encoding(self, input_data: str) -> Optional[SecurityViolation]:
        ...
        try:
            ...
            return SecurityViolation(
                ...
                confidence_score=1.0,
                location=f"byte_position:{e.start}-{e.end}" if hasattr(e, 'start') else None,
    ...
    def _detect_with_fallback_patterns(self, input_data: str) -> List[SecurityViolation]:
        ...
        for pattern_regex, pattern_name, confidence in self._compiled_patterns:
            ...
            for match in matches:
                violation = SecurityViolation(
                    ...
                    severity=SecuritySeverity.ERROR if confidence >= 0.9 else SecuritySeverity.WARNING,
    ...
    def _determine_recommended_action(self, violations: List[SecurityViolation]) -> SecurityAction:
        """Determine recommended action based on violations."""
        if not violations:
            return SecurityAction.ALLOW
        
        # Check for critical violations
        critical_violations = [v for v in violations if v.severity == SecuritySeverity.CRITICAL]
        ...
        sanitizable_violations = [
            v for v in violations 
            if v.violation_type in ['xss_injection', 'html_injection', 'script_injection']
        ]
    ...
    async def check_library_health(self) -> LibraryHealthStatus:
        ...
        if LIBINJECTION_AVAILABLE:
            try:
                ...
                self.library_health.health_check_errors.append(f"libinjection test failed: {e}")
        ...
        if BLEACH_AVAILABLE:
            try:
                ...
                self.library_health.health_check_errors.append(f"bleach test failed: {e}")
        ...
        if MARKUPSAFE_AVAILABLE:
            try:
                ...
                self.library_health.health_check_errors.append(f"markupsafe test failed: {e}")
        
        self.library_health.last_health_check = current_time
        
        if self.library_health.health_check_errors:
            logger.warning(f"Security library health issues: {'; '.join(self.library_health.health_check_errors)}")
        else:
