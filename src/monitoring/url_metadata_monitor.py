...
class URLMetadataMonitor:
    ...
    def record_upload(self, url: str, success: bool, duration_ms: float,
                     metadata_size: int, error_type: Optional[str] = None,
                     correlation_id: Optional[str] = None):
    ...
    def _calculate_failure_rate(self, operation_type: str, 
                               cutoff_time: datetime) -> Optional[float]:
        """Calculate failure rate for an operation type."""
        with self._lock:
            metrics = [
                m for m in self.metrics_store.get(operation_type, [])
    ...
    def _calculate_response_time_percentile(self, percentile: int,
                                          cutoff_time: datetime) -> Optional[float]:
    ...
    def _calculate_broken_links_rate(self, cutoff_time: datetime) -> Optional[float]:
        ...
        with self._lock:
            validation_metrics = [
                m for m in self.metrics_store.get('validation', [])
        ...
        broken_count = sum(
            1 for m in validation_metrics
            if m.error_type == 'broken_link'
        )
    ...
    def check_url_accessibility(self, url: str) -> Tuple[bool, Optional[str]]:
        ...
        if not is_accessible:
            self.record_validation(
                url, False, 0, error_type='broken_link'
            )
    ...
    def _calculate_performance_distribution(self, 
                                          cutoff_time: datetime) -> Dict[str, int]:
...
def record_upload(url: str, success: bool, duration_ms: float,
                 metadata_size: int, **kwargs):
