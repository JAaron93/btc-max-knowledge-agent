...
async def test_tts_circuit_breaker_integration():
    ...
    try:
        ...
        from src.utils.tts_error_handler import CircuitBreakerConfig, TTSCircuitOpenError
    ...
    with patch('src.utils.tts_service.get_audio_cache') as mock_cache:
        ...
        tts_service.error_handler.circuit_breaker.request_window.maxlen = circuit_config.window_size
        ...
        with patch.object(tts_service, '_synthesize_with_api', side_effect=Exception("API Error")):
        ...
        with patch.object(tts_service, '_synthesize_with_api', return_value=b"fake_audio_data"):
            try:
                ...
                if circuit_state == "closed":
                    ...
                    logger.warning(f"⚠ Circuit state is {circuit_state}, expected 'closed'")
        ...
        if error_state["circuit_breaker"]["state"] == "open":
            ...
            if error_state["circuit_breaker"]["state"] == "closed":
                ...
                logger.warning(f"⚠ Circuit state after reset: {error_state['circuit_breaker']['state']}")
